...
int main(int argc, char* argv[]) {
    ...
    cdcOutChunk = (char *)malloc(sizeof(char) * Max_Chunk_Size);
    deDupOutChunk = (char *)malloc(sizeof(char) * Max_Chunk_Size);
    deDupInChunk = (char *)malloc(sizeof(char) * Max_Chunk_Size);
...
/*     uint16_t *chunk_size = (uint16_t *)malloc(sizeof(uint16_t) * MAX_BOUNDARY);
    ...
    char* pipeline_drained = (char *)malloc(sizeof(char));
    uint16_t* offset_buff = (uint16_t *)malloc(sizeof(uint16_t));
    uint32_t* deDup_header = ((uint16_t *)malloc(sizeof(uint32_t));
                              
    //void core_1_process(unsigned char *buff, unsigned int buff_size, char* chunk, uint16_t *chunk_size, uint16_t *offset_buff, char *pipeline_drained, unsigned char *output) {
        //cdc(buff, buff_size, chunk, chunk_size, offset_buff, pipeline_drained);
        //output = chunk;
        
    //}
                              
    void core_2_process(char* inputChunk, uint16_t chunk_size,
                        std::unordered_map<string, uint32_t>& chunkTable, uint32_t* header,
                        stopwatch& stopwatch) {
        deDup(inputChunk, chunk_size, chunkTable, header, stopwatch);
        //need to wait for core 1 process
    }
    ...
    /*
            ...
            *offset_buff = 0; //refresh offset_buff to 0
        ...
        while(*pipeline_drained == 0) {
            ...
            if (count == 2) {
                ...
                cdc_timer.start();
                cdc_thread = std::thread(&cdc, file, offset, cdcOutChunk, chunk_size, offset_buff, pipeline_drained);   //boundary_num should use char?
                pin_thread_to_cpu(cdc_thread, 1);
                cdc_timer.stop();
                ...
                offset += length;
                cdc_timer.start();
                cdc_thread = std::thread(&cdc, &buffer[2], length, cdcOutChunk, chunk_size, offset_buff, pipeline_drained);   //boundary_num should use char?
                pin_thread_to_cpu(cdc_thread, 1);
                cdc_timer.stop();
            }
            // printf("after cdc\n");
            ...
            if (loop_count > 0) {
                deDup_timer.start();
                deDupInChunk = cdcOutChunk;
                deDup_thread = std::thread(&deDup, deDupInChunk, chunk_size, chunkTable, deDup_header, std::ref(SHA_timer));
                pin_thread_to_cpu(cdc_thread, 2);
                deDup_timer.stop();
