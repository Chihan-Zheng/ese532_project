...
// ==============================================================
`timescale 1ns/1ps
module LZW_hybrid_hash_HW_control_s_axi
#(parameter
...
always @(posedge ACLK) begin
    if (ARESET)
        int_send_data[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_SEND_DATA_DATA_0)
            int_send_data[31:0] <= (WDATA[31:0] & wmask) | (int_send_data[31:0] & ~wmask);
    end
end
