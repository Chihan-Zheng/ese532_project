...
#define Max_Chunk_Size (pow(2, CODE_LEN))
...
//****************************************************************************************************************
void hardware_encoding(char in[MAX_CHUNK], uint16_t in_length, uint16_t send_data[Max_Chunk_Size + 2])
{
    ...
    //-------------------------------end hash_lookup-----------------------------------
        ...
        if(!hit)
        {
            ...
            if (j == 0){   //the first code
                shift = 3;
                store_array[0] = prefix.to_uint16() << shift;
                j++;
            }else{       
                if (shift < 13){      //check whether empty space of store_array[j-1] is greater than 13
                    shift = shift + 3;
                    store_array[j] = prefix.to_uint16() << shift;     //xxxxx00 yyyyyyyy -> xxxxx|yy yyyyyy00 (shift here is 2)
                    shift = 16 - shift;      
                    store_array[j-1] = store_array[j-1] | (prefix.to_uint16() >> shift);
                    shift = 16 - shift;
                    ...
                    store_array[j-1] = store_array[j-1] | (prefix.to_uint16() << vacant_bit_number);
    ...
    if (in_length == 1) {      //if chunk length is 1: a single character
        shift = shift + 3;
        store_array[j] = prefix.to_uint16() << shift;
        ...
        if (shift < 13){ 
            ...
            store_array[j] = swap_endian_16(store_array[j]);
            shift = 16 - shift;
            store_array[j-1] = store_array[j-1] | (prefix.to_uint16() >> shift);        //store the last part of in
            store_array[j-1] = swap_endian_16(store_array[j-1]);
            shift = 16 - shift;
        }else{
